...
class MeshFrame():
    ...
    def nSideBase3Dunit(self,params=None,n_sides=8):
        ...
        xy_squeez = params.get('xy_squeez', 1.0)
        R = B/np.sqrt(2)
        ...
        x = xy_squeez * R * np.cos(angle[0:-1])
        y = xy_squeez * R * np.sin(angle[0:-1])
        ...
        vertices[n_sides:int(n_sides*2),:] = (np.dot(rotCord(np.pi, axis='y'), vertices[0:n_sides,:].T)).T
        vertices[int(n_sides*2):int(n_sides*3),:] = (np.dot(rotCord(np.pi/2, axis='y'), vertices[0:n_sides,:].T)).T
        vertices[int(n_sides*3):int(n_sides*4),:] = (np.dot(rotCord(-np.pi/2, axis='y'), vertices[0:n_sides,:].T)).T
        vertices[int(n_sides*4):int(n_sides*5),:] = (np.dot(rotCord(np.pi/2, axis='x'), vertices[0:n_sides,:].T)).T
        vertices[int(n_sides*5)::,:] = (np.dot(rotCord(-np.pi/2, axis='x'), vertices[0:n_sides,:].T)).T
        ...
        for i in range(5):
            connMat = np.concatenate((connMat,connMatbase+n_sides*(i+1))).astype(int)
        ...
        def find_close_pairs(vertices,arr1,arr2):
            ...
            diff_vectors = (one_octagon - rest_octagons[:,np.newaxis,:])
            distances = np.sqrt(np.sum(diff_vectors**2, axis=2))
        ...
        for i in range(4): # top, bot, left and right side connections
            ...
            connMat = np.concatenate((connMat,conn)).astype(int)
    ...
    def combined3Dunit(self,params=None):
        ...
        if params['Name'][0] == 'C':
            ...
            n_Sides = int(params['Name'][1:])
            vertices,connMat = self.nSideBase3Dunit(params,n_Sides)
            params['base'] = params['base']*2
        ...
        for k in range(nz):
            for j in range(ny):
                ...
                for i in range(nx + int(2 * etr)):
                    ...
                    # Mov2 = (RotM @ Mov1.T).T
                    Mov2 = np.dot(RotM, Mov1.T).T  # Apply rotation to the vertices
    ...
    def merge_vertices_and_conn(self,vertices_list, connMat_list):
        ...
        # data_to_unique = all_vertices.round(decimals=10)
        # dt = np.dtype((np.void, data_to_unique.dtype.itemsize * data_to_unique.shape[1]))
        # view = np.ascontiguousarray(data_to_unique).view(dt)
        # _, unique_indices_into_view, unique_inverse_map = np.unique(view, return_index=True, return_inverse=True)
        # unique_vertices = data_to_unique[unique_indices_into_view]
        # unique_indices = unique_inverse_map
